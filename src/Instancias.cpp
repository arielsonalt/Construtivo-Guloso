...
void Instancias::imprimirDistancias() {
...
void Instancias::imprimirClientes() {
...
void Instancias::lerDistancias(){
    xml_document<> doc;
	xml_node<> * root_node;
	ifstream theFile ("distancias_cts.xml");
	vector<char> buffer((istreambuf_iterator<char>(theFile)), istreambuf_iterator<char>());
	buffer.push_back('\0');
	doc.parse<0>(&buffer[0]);
	...
	for (xml_node<> * cidade_node = root_node->first_node("cidade");
        ...
        cidade_node = cidade_node->next_sibling()){
            ...
            for (unsigned int i=1; i<=52; i++) {
                ...
                //cout << "Distancia ct" << i << ": "<<ct_node->value() << endl;
            }
        vetorDistancias.push_back(tmp);
    }
}

void Instancias::lerClientes(){
    xml_document<> doc;
	xml_node<> * root_node;
	ifstream theFile ("clientes.xml");
	vector<char> buffer((istreambuf_iterator<char>(theFile)), istreambuf_iterator<char>());
	buffer.push_back('\0');
	doc.parse<0>(&buffer[0]);
	root_node = doc.first_node("data-set");
